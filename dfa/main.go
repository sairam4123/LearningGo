...
func (t Transition) WithIp(input string) Transition {
	return Transition{
		Name:         t.Name,
		Input:        input,
		Next:         t.Next,
...
func (t *Transition) GetName() string {
	var tName = t.Name
	if !t.IsCustomName {
		tName = t.Current.GetName() + "--" + t.Input + "->" + t.Next.GetName()
	}
	return tName
}
...
func (d DFA) GetBestTransition(curState *State, input string) (Transition, error) {
...
func (d DFA) Run(input string) (DFAResult, error) {
	...
	for _, ip := range input {
		var trans, err = d.GetBestTransition(&curState, string(ip))
		if err != nil {
...
func (d DFA) GetInputTransitionStates(curState *State, input string) []*State {
	var reachableStates = make([]*State, 0)
	for _, v := range d.transitions {
		if v.Current.GetName() == curState.GetName() && v.Input == input {
			reachableStates = append(reachableStates, v.Next)
...
func main() {
	...
	var d, derr = DFA{}.AddState(&q1, &q2, &q3, &q4, &q5, &q6).AddTransition(t1, t2, t3, t4, t6, t7, t8, t9).Validate()
	if derr != nil {
		fmt.Println("DFA is invalid", derr)
		return
	}
	var res, err = d.Run("bbdd")
